...
UPDATE gadm3 SET fulltext_search_1 =
  ...
  || array_to_tsvector(array_remove(string_to_array(unaccent(coalesce(varname_1, '')), '|'), ''))
  || array_to_tsvector(array_remove(string_to_array(unaccent(coalesce(nl_name_1, '')), '|'), ''));
...
UPDATE gadm3 SET fulltext_search_2 =
  ...
  || array_to_tsvector(array_remove(string_to_array(unaccent(coalesce(varname_2, '')), '|'), ''))
  || array_to_tsvector(array_remove(string_to_array(unaccent(coalesce(nl_name_2, '')), '|'), ''));
...
UPDATE gadm3 SET fulltext_search_3 =
  ...
  || array_to_tsvector(array_remove(string_to_array(unaccent(coalesce(varname_3, '')), '|'), ''))
  || array_to_tsvector(array_remove(string_to_array(unaccent(coalesce(nl_name_3, '')), '|'), ''));
...
SELECT DISTINCT ON(gid_0)
  gid_0 AS gid, name_0 AS name, array_remove(string_to_array(varname_0, '|'), '') AS variant_name, NULL::text[] AS non_latin_name, ARRAY['Country or area'] AS type, ARRAY['Country or area']  AS english_type,
...
SELECT DISTINCT ON(gid_1)
  gid_1 AS gid, name_1 AS name, array_remove(string_to_array(varname_1, '|'), '') AS variant_name, array_remove(string_to_array(nl_name_1, '|'), '') AS non_latin_name, array_remove(string_to_array(type_1, '|'), '') AS type, array_remove(string_to_array(engtype_1, '|'), '') AS english_type,
...
SELECT DISTINCT ON(gid_2)
  gid_2 AS gid, name_2 AS name, array_remove(string_to_array(varname_2, '|'), '') AS variant_name, array_remove(string_to_array(nl_name_2, '|'), '') AS non_latin_name, array_remove(string_to_array(type_2, '|'), '') AS type, array_remove(string_to_array(engtype_2, '|'), '') AS english_type,
...
SELECT DISTINCT ON(gid_3)
  gid_3 AS gid, name_3 AS name, array_remove(string_to_array(varname_3, '|'), '') AS variant_name, array_remove(string_to_array(nl_name_3, '|'), '') AS non_latin_name, array_remove(string_to_array(type_3, '|'), '') AS type, array_remove(string_to_array(engtype_3, '|'), '') AS english_type,
