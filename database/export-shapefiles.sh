...
function export_natural_earth() {
	...
	exec_pgsql2shp layers/political "SELECT iso_a2 AS id, name AS name, iso_a2 AS isoCountryCode2Digit, geom FROM political"
...
function export_marine_regions() {
	...
	exec_pgsql2shp layers/eez "SELECT mrgid AS id, geoname AS name, CONCAT_WS(' ', im1.iso2, im2.iso2, im3.iso2) AS isoCountryCode2Digit, geom FROM eez LEFT OUTER JOIN iso_map im1 ON eez.iso_ter1 = im1.iso3 LEFT OUTER JOIN iso_map im2 ON eez.iso_ter2 = im2.iso3 LEFT OUTER JOIN iso_map im3 ON eez.iso_ter3 = im3.iso3"
...
function export_gadm() {
	...
	exec_pgsql2shp layers/gadm "SELECT gid_0, gid_1, gid_2, gid_3, name_0, name_1, name_2, name_3, iso_map.iso2 AS isoCountryCode2Digit, geom FROM gadm3 LEFT OUTER JOIN iso_map ON gadm3.gid_0 = iso_map.iso3"
...
function export_iho() {
	...
	exec_pgsql2shp layers/iho "SELECT 'http://marineregions.org/mrgid/' || mrgid AS id, name, NULL AS isoCountryCode2Digit, geom FROM iho"
...
function export_wgsrpd() {
	...
	exec_pgsql2shp layers/wgsrpd "SELECT 'WGSRPD:' || level4_cod AS id, level_4_na AS name, iso_code AS isoCountryCode2Digit, geom FROM wgsrpd_level4"
...
function export_continents() {
	...
	exec_pgsql2shp layers/continents "SELECT continent AS id, continent AS name, NULL AS isoCountryCode2Digit, geom FROM continent"
