...
SELECT DISTINCT
  ...
  0 AS gadm_level, NULL::text[] AS top_levels, NULL::hstore AS top_levels_map, NULL AS parent_gid, fulltext_search_0 AS fulltext_search FROM gadm3
UNION ALL
SELECT DISTINCT
  id_1 AS id, gid_1 AS gid, name_1 AS name, string_to_array(varname_1, '|') AS variant_name, string_to_array(nl_name_1, '|') AS non_latin_name, string_to_array(type_1, '|') AS type, string_to_array(engtype_1, '|') AS english_type,
  1 AS gadm_level, ARRAY[gid_0] AS top_levels, hstore(gid_0, name_0) AS top_levels_map,  gid_0 AS parent_gid, fulltext_search_1 AS fulltext_search FROM gadm3
UNION ALL
SELECT DISTINCT
  id_2 AS id, gid_2 AS gid, name_2 AS name, string_to_array(varname_2, '|') AS variant_name, string_to_array(nl_name_2, '|') AS non_latin_name, string_to_array(type_2, '|') AS type, string_to_array(engtype_2, '|') AS english_type,
  2 AS gadm_level, ARRAY[gid_0, gid_1] AS top_levels, hstore(gid_0,name_0) || hstore(gid_1, name_1) AS top_levels_map, gid_1 AS parent_gid, fulltext_search_2 AS fulltext_search FROM gadm3
UNION ALL
SELECT DISTINCT
